...
def test_admin_can_create_category_for_a_specific_branch(admin_client, restaurant, branch):
    """2026-09-03 - found while rebuilding test data: an Admin (no fixed
    branch of their own) specifying ?branch= in the request body used to
    have it silently discarded and replaced with None."""
    _, client = admin_client

    response = client.post(
        "/v1/menu/categories/", {"name": "Starters", "branch": str(branch.id), "sort_order": 1}, format="json",
    )
...
def test_admin_can_create_same_category_name_on_two_different_branches(admin_client, restaurant):
    from apps.restaurant.models import Branch
    ...
    r1 = client.post("/v1/menu/categories/", {"name": "Starters", "branch": str(branch_a.id), "sort_order": 1}, format="json")
    r2 = client.post("/v1/menu/categories/", {"name": "Starters", "branch": str(branch_b.id), "sort_order": 1}, format="json")

    assert r1.status_code == 201, r1.data
    assert r2.status_code == 201, r2.data  # different branches - not a real conflict


def test_manager_cannot_create_category_for_another_branch(manager_client, restaurant, branch):
    """A Manager is always pinned to their own branch - the request body
    can't override it, even if they try to specify a different one."""
    from apps.restaurant.models import Branch
    ...
    response = client.post(
        "/v1/menu/categories/", {"name": "Starters", "branch": str(other_branch.id), "sort_order": 1}, format="json",
    )
...
def test_category_branch_rejects_another_restaurants_branch(admin_client):
    from apps.restaurant.models import Branch, Restaurant

    other_restaurant = Restaurant.objects.create(name="Other Restaurant", slug="other-restaurant-cat")
    foreign_branch = Branch.objects.create(restaurant=other_restaurant, name="Foreign Branch")
    _, client = admin_client

    response = client.post(
        "/v1/menu/categories/", {"name": "Starters", "branch": str(foreign_branch.id), "sort_order": 1}, format="json",
    )
